"""
khalifa_stewardship_tour_operating_system/services/agent_bridge.py — bridges
the real capital-at-risk / recoverable-value formulas to the Agent Runtime &
Model Router's execution pipeline, for the Kazakhstan Clean Heat Stewardship
Tour flagship case. Mirrors financial_intelligence_cloud's own
services/agent_bridge.py exact shape — this app never re-derives the
arithmetic itself, it imports and calls the real functions.

Reference worked example (Kazakhstan Clean Heat golden case):
inventory_value=1200, historical_loss_rate=0.35 -> capital_at_risk=420.0;
expected_value_recovered=650, intervention_cost=150 -> recoverable_value=500.0.
"""
...
def build_waste_leakage_fixture(inventory_value, historical_loss_rate, evidence_used, missing_data,
    ...
    output_summary = (
        f'Projected capital at risk: £{capital_at_risk:,.0f}/year. '

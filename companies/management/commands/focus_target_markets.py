...
Actions (all idempotent):
  1. Seeds ~60 Türkiye base company profiles.
  2. Seeds 193 additional companies across UK/KZ/SA/TR (from _target_market_data.py).
  3. Sets CompanyProfile.status = 'archived' for every company whose
     country is NOT in {United Kingdom, Kazakhstan, Saudi Arabia, Türkiye}.

Target distribution after a full run (with add_400_companies + seed_global_companies):
  United Kingdom  ~100
  Kazakhstan      ~100
  Saudi Arabia    ~100
  Türkiye         ~100
  All others         0  (archived)

Usage:
    python manage.py focus_target_markets
    python manage.py focus_target_markets --overwrite   # re-seed all managed companies
"""
import random
...
class Command(BaseCommand):
    help = (
        'Seed all target-market companies and archive non-target profiles. '
        'Targets: United Kingdom, Kazakhstan, Saudi Arabia, Türkiye (~100 each).'
    )

    def add_arguments(self, parser):
        parser.add_argument(
            '--overwrite', action='store_true',
            help='Update scores for already-existing managed companies',
        )

    # ── internal helper ───────────────────────────────────────────────────────
    def _seed_batch(self, companies, overwrite, batch_label):
        """Seed a list of (name, sector, country, target, harm_level) tuples.
        ...
        for (name, sector, country, target, harm_level) in companies:
            ...
            if not co_created and overwrite:
                company.name = name; company.sector = sector; company.country = country
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"  {verb:7s} [{country[:16]:16s}] {name[:38]:38s} "
    ...
    def handle(self, *args, **options):
        ...
        # ── Step 1: Seed Türkiye base companies (~60) ─────────────────────────
        self.stdout.write(self.style.MIGRATE_HEADING(
            f'─── Step 1: Seeding Türkiye base companies ({len(TURKEY_COMPANIES)}) ───'
        ))
        c1, u1, s1 = self._seed_batch(TURKEY_COMPANIES, overwrite, 'Türkiye base')
        self.stdout.write(self.style.SUCCESS(
            f'  Done. Created={c1}  Updated={u1}  Skipped={s1}'
        ))

        # ── Step 2: Seed additional companies for all 4 markets (193) ─────────
        self.stdout.write(self.style.MIGRATE_HEADING(
            f'\n─── Step 2: Seeding additional companies ({len(ADDITIONAL_COMPANIES)}) ───'
        ))
        c2, u2, s2 = self._seed_batch(ADDITIONAL_COMPANIES, overwrite, 'Additional')
        self.stdout.write(self.style.SUCCESS(
            f'  Done. Created={c2}  Updated={u2}  Skipped={s2}'
        ))

        # ── Step 3: Archive non-target-market profiles ────────────────────────
        self.stdout.write(self.style.MIGRATE_HEADING(
            '\n─── Step 3: Archiving non-target-market profiles ───'
        ))
        ...
        total_public = sum(dist.values())
        self.stdout.write(self.style.SUCCESS(
            f'\n  Total public/verified: {total_public}'
        ))
        self.stdout.write(self.style.SUCCESS('focus_target_markets complete.'))
